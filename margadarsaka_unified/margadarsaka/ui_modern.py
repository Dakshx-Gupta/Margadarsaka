...
def initialize_custom_components():
    """Initialize and load all custom components"""
    # Create a container for initialization (won't be visible)
    with st.container():
        st.markdown("### Initializing custom components")
        # Initialize each component type to ensure they're loaded
        rating = rating_component(key="init_rating", initial_value=0)
