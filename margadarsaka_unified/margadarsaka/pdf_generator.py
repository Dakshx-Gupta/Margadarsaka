...
class RoadmapVisualizer:
    ...
    def create_career_roadmap_pdf(
        ...
        for i, career_path in enumerate(career_paths[:3], 1):  # Top 3 paths
            story.append(
                Paragraph(
                    f'{i}. {career_path.role or "N/A"}',
                    styles["Heading2"],
                )
            )
            story.append(
                Paragraph(
                    f"Match Score: {career_path.score * 100:.1f}%", styles["Normal"]
                )
            )
            story.append(
                Paragraph(career_path.description or "N/A", styles["Normal"])
            )
            ...
            if career_path.salary_range:
                story.append(
                    Paragraph(
                        f"Salary Range: ₹{career_path.salary_range}",
                        styles["Normal"],
                    )
                )
    ...
    def _generate_text_summary(
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            for i, path in enumerate(career_paths[:3], 1):
                f.write(
                    f"{i}. {path.role or 'N/A'} (Match: {path.score * 100:.1f}%)\n"
                )
                f.write(f"   {path.description or 'N/A'}\n\n")
