...
class CareerRecommendationEngine:
    ...
    def _extract_keywords_from_path(self, path_data: Dict) -> Set[str]:
        ...
        role_words = re.findall(r"\w+", path_data.get("role", "").lower())
        industry_words = re.findall(r"\w+", path_data.get("industry", "").lower())
    ...
    def _recommend_learning_resources(
        ...
        for resource_data in self.resources:
            ...
            if profile.experience_years is not None:
                resource_level = resource_data.get("level", "beginner")
                if profile.experience_years < 2 and resource_level == "beginner":
                    relevance_score += 0.2
                elif (
                    2 <= profile.experience_years < 5 and resource_level == "intermediate"
                ):
                    relevance_score += 0.2
                elif profile.experience_years >= 5 and resource_level == "advanced":
                    relevance_score += 0.2
            ...
            if relevance_score > 0.3:  # Threshold for relevance
                resource = Resource(
                    ...
                    duration=resource_data.get("duration", "N/A"),
                )
    ...
    def _get_mentorship_resources(self, profile: UserProfile) -> List[Resource]:
        ...
        return [
            Resource(
                ...
                description=resource_data.get("description", ""),
                tags=resource_data.get("tags", []),
                resource_type=resource_data.get("type", "community"),
                level=resource_data.get("level", "all"),
                provider=resource_data.get("provider", ""),
                cost=resource_data.get("cost", "Free"),
                duration=resource_data.get("duration", "Ongoing"),
            )
            for resource_data in self.mentorship_resources
        ]
    ...
    def _get_job_resources(self, profile: UserProfile) -> List[Resource]:
        ...
        return [
            Resource(
                ...
                description=resource_data.get("description", ""),
                tags=resource_data.get("tags", []),
                resource_type=resource_data.get("type", "job_board"),
                level=resource_data.get("level", "all"),
                provider=resource_data.get("provider", ""),
                cost=resource_data.get("cost", "Free"),
                duration=resource_data.get("duration", "N/A"),
            )
            for resource_data in self.job_resources
        ]
                ...
                if category == "all" or resource.get("category") == category:
                    roadmap_resources.append(
                        Resource(
                            ...
                            resource_type=resource.get("type", "roadmap"),
                            level=resource.get("level", "all"),
                            provider=resource.get("provider", "roadmap.sh"),
